...
- **MOBILE PENDING (owner pairing rule — must ship together):** build `frontend/src/mobile/MobileAssistant.tsx` reusing `AssistantChat`; wire into `frontend/src/mobile/MobileApp.tsx` (add an `assistant` to the `Screen` union ~:70-101 + `destinationScreen`, OR a floating launcher via the `annPopup` precedent rendered in both returns ~:751 & ~:873), gated by `canUseAssistant(user)`; styles in `frontend/src/mobile/mobile.css` (mobile is plain CSS, NOT Tailwind). ~~Mobile has ZERO assistant surface today.~~ **FALSE — it shipped.** `frontend/src/mobile/MobileAssistant.tsx:31` exports it, gated by `canUseAssistant`, sharing `useAssistantChat` with desktop; `MobileApp.tsx:71` imports it and `:461` renders it. This block reads as an open build task with a full spec, so the next reader builds it a second time. The status table above already says DONE — the file contradicted itself and THIS was the wrong half.
