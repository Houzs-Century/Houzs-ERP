...
183 route modules (51 in `backend/src/routes`, 132 in `backend/src/scm/routes`), 1253 endpoint registrations.
...
| `backend/src/scm/routes/accounting-bank-months.ts` | 0 | 503 |
| `backend/src/scm/routes/accounting-bank.ts` | 0 | 1266 |
...
| `backend/src/scm/routes/accounting.ts` | 89 | 1550 |
...
| `backend/src/db/migrations-pg` | `backend/scripts/pg-migrate.mjs` | 403 | `0352_acc_pv_files.sql` (0352) | YES | no |
...
Top 20 by line count across `backend/src` and `frontend/src` (2180 files, 746868 lines total).
