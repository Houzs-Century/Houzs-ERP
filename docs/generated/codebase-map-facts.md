...
194 route modules (53 in `backend/src/routes`, 141 in `backend/src/scm/routes`), 1295 endpoint registrations.
...
| `backend/src/scm/routes/accounting-bank.ts` | 0 | 1448 |
...
Top 20 by line count across `backend/src` and `frontend/src` (2462 files, 791666 lines total).
