...
170 route modules (46 in `backend/src/routes`, 124 in `backend/src/scm/routes`), 1189 endpoint registrations.
...
| `backend/src/routes/announcements.ts` | 15 | 1883 |
...
| `backend/src/scm/routes/accounting.ts` | 74 | 1446 |
...
| `backend/src/db/migrations-pg` | `backend/scripts/pg-migrate.mjs` | 367 | `0352_acc_pv_files.sql` (0352) | YES | no |
...
Top 20 by line count across `backend/src` and `frontend/src` (1980 files, 697777 lines total).
...
| `frontend/src/mobile/MobileAnnouncements.tsx` | 1490 |
