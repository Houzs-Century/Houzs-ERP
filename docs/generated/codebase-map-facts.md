...
178 route modules (49 in `backend/src/routes`, 129 in `backend/src/scm/routes`), 1227 endpoint registrations.
...
| `backend/src/scm/routes/accounting-bank-months.ts` | 0 | 309 |
...
| `backend/src/db/migrations-pg` | `backend/scripts/pg-migrate.mjs` | 386 | `0352_acc_pv_files.sql` (0352) | YES | no |
...
Top 20 by line count across `backend/src` and `frontend/src` (2073 files, 720562 lines total).
