...
176 route modules (47 in `backend/src/routes`, 129 in `backend/src/scm/routes`), 1219 endpoint registrations.
...
| `backend/src/routes/publicContractorCalendar.ts` | 3 | 300 |
...
| `backend/src/scm/routes/accounting-bank-months.ts` | 0 | 267 |
| `backend/src/scm/routes/accounting-bank.ts` | 0 | 803 |
...
| `backend/src/scm/routes/accounting.ts` | 79 | 1516 |
...
| `backend/src/scm/routes/document-cancel-routes.ts` | 11 | 570 |
...
| `backend/src/scm/routes/so-handover.ts` | 2 | 242 |
...
| `backend/src/db/migrations-pg` | `backend/scripts/pg-migrate.mjs` | 384 | `0352_acc_pv_files.sql` (0352) | YES | no |
...
Top 20 by line count across `backend/src` and `frontend/src` (2066 files, 719759 lines total).
