...
154 route modules (44 in `backend/src/routes`, 110 in `backend/src/scm/routes`), 1123 endpoint registrations.
...
| `backend/src/scm/routes/accounting.ts` | 53 | 1256 |
...
| `backend/src/scm/routes/payment-vouchers.ts` | 14 | 1689 |
...
| `backend/src/db/migrations-pg` | `backend/scripts/pg-migrate.mjs` | 343 | `0343_scm_pv_checked.sql` (0343) | YES | no |
...
Top 20 by line count across `backend/src` and `frontend/src` (1862 files, 670245 lines total).
