...
> **更新（2026-09-07 22:21 +08）：** 本文的 AutoCount 快照是当天 13:03 那一份。
> 当晚 22:21 又从现场重新导了一次，用另一支工具
> （`check-stock-vs-autocount.mjs`）把**库存数量**逐个「货品 × 仓库」对了一遍，
> 结果和分类写在 [`stock-vs-autocount-2026-09-07.md`](./stock-vs-autocount-2026-09-07.md)。
> 重点：996 个格子 789 个一样，对不上的 202 个里面**真正导错的只有 4 件货**，其余
> 都是 8 月 29 日之后 AutoCount 继续做单、我们没接上。两份文件的张数/格子数不能
> 直接相比 —— 一份数的是「单」，一份数的是「货品 × 仓库」，口径不同。
