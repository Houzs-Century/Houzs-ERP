...
| **SKU-rename cascade vs `sku-usage.ts`** | `lib/product-code-rename.ts` lists the (table, column) pairs a rename re-points and the code columns that keep their value, pinned against production by `tests/productCodeRenameCoverage.test.ts` (2026-09-15); `lib/sku-usage.ts:15` lists 3 of them; `lib/autocount-outbox.ts:870` maps 6 line tables to their code column. | Three questions (rename everywhere / has-it-been-used / what does AutoCount call it), not one list. Worth a shared table→column map eventually. |
