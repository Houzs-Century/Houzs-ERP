...
acceptance query is already written in our own code (`AcSyncService.cs:320-322`):
...
| Line identity now exists for most documents | SO **12,910 / 13,909 (92.8%)**, PO **275 / 864**. **2,316 of 2,723** SOs and **127 of 449** POs are fully covered, meaning editable |
...
|---|---|---|---|---|
| **SO** | BUILT | n/a (no parent) | BUILT (was NOTHING) | BUILT |
| **PO** | BUILT | n/a (no parent) | BUILT (was NOTHING) | BUILT |
...
| 4 | **An uncompilable over-transfer handler** (`fix/acsync-overqty-uncompilable`) | The SDK raises a WinForms dialog for over-transfer. The service carried a handler to answer it programmatically. That event's `EventArgs` type is **not public**, so the handler could never have compiled at all | Reading the reflected SDK surface rather than the SDK docs. Replaced by making the condition unreachable — only ever transfer what is outstanding |
