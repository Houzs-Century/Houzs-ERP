...
| `requeued-with-parent` | **yes** | The row said the document had **no earlier document to carry across**, and that was wrong — its lines do come from one. The parent was read afresh from the document and a **real conversion** was composed and queued. Anything above it that was not in the account book went first. | Nothing. This is the answer for a receipt or supplier invoice that had been sitting with no button on it. |
...
| `host-unreachable` | The request never reached the AutoCount machine. Cloudflare answered for an origin it could not reach, so **nothing was sent and nothing was refused** — the account book is untouched. Listed BEFORE `masters-not-opened` because the masters call is where this surfaces, so the stored sentence carries both needles and this one must win. | Yes, the moment the machine answers. | `curl -i https://autocount.houzscentury.com/health` — 200 means it is back, 502 means it is not. Then Send again. |
...
| `line <DtlKey> not found on <docNo>` | `AcSyncService.cs` | A keyed `edit` named a line the book no longer holds under that key. A `rebuild` deletes the book's lines and re-adds them with fresh keys, and an edit composed against the OLD keys before it drained names lines that are gone. | **It heals itself now, on the common case.** | Nothing: the ERP composes a fresh edit with the CURRENT keys and folds this row as Replaced (`autocount-stale-key-recompose.ts`, docs/bugs/0941). The exception is when the ERP STILL carries the refused key — then the account book lost the line, not the ERP, and a person re-adds it in AutoCount. |
