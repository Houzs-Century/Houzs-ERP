...
- A role's flat `permissions` only decides what it can DO — page access (what it can SEE) comes from the member's Title (`positionPolicy.ts`, name-keyed cohorts: god / full / restricted / sales). The Role-settings drawer's page matrix (`role_page_access`) is read ONLY for a member with no Title; the drawer says so. Editing the Roles matrix never touches menu visibility.
