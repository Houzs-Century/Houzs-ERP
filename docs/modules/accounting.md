...
- Other Debtor bill: posts on create (atomic with its journal); PAID when fully knocked off; cancel refused once money received; edit re-posts; prints as an INVOICE to the debtor (description and amount per line, never the account code; total, received, balance due, words; CANCELLED watermark). The registry holds the party's data as a supplier's — TIN, business reg no, contact person, attention, phones, email, fax, four address lines, city, postcode, state, country (`debtor-party.ts`; one pop-out form for New and Edit) — and BILL TO prints the address, attention, phone, email, TIN and reg no. A line with a description and no amount is a text line (no account, `credit_account_code` NULL): it prints, books nothing, never counts in the total; a bill needs at least one money line. The foot prints the company's Other Debtor payment details and terms from Settings › Branding (`debtorPaymentDetails`, `debtorInvoiceTerms`); the Deposit Invoice and the Sales Invoice print the customer set (`customerPaymentDetails`); a blank setting prints nothing.
...
- Deposit invoice (DI): issued and posted per qualifying customer payment; closed by one CN when the order's final invoice posts (`credit_note_id`); cancel needs a reason (row kept); `POST /deposit-invoices/:id/post` retries a journal refused at birth. An invoice with a credit note against it (close-out, refund or conversion note) is never cancelled or re-issued: a payment edit or delete re-posts the payment alone and leaves the invoice, the page refuses `invoice_noted`; cancel the final invoice or the refund voucher first.
...
- Forecast P&L (`GET`/`PUT /accounting/forecast`, `GET /accounting/forecast/figures?from&to`; `scm.acc_forecast_pnl`, one row per month per company, `lines` jsonb code → `{ bp }` | `{ amtSen }`): the rows are the P&L's active leaf accounts on the P&L layout; a sales line takes an amount (the month's forecast sales are their sum), every other line a share of that in basis points or an amount, never both; a whole-grid PUT names the first bad cell and removes absent months; the figures follow the P&L's blocks via the shared `scm/shared/forecast-pnl.ts` (vendored byte-identical); nothing posts. Read = `scm.payment_voucher.post`, write = `scm.payment_voucher.write`. The page is `/scm/forecast` (Finance › Forecasting; `ForecastPnl.tsx`): months across, the P&L's tree down (an account printed `code · name` on one line, the report trees' own label), two boxes a cell (RM | % sales — key one, the other shows what it implies), category subtotals and the statement's totals, Add month inherits the percentages, Hide / Show empty lines, one Save of the whole grid that scrolls to a refused cell.
- Financial Dashboard (`GET /accounting/dashboard?granularity=month|quarter&periods=N&from=YYYY-MM&to=YYYY-MM`; read = `scm.payment_voucher.post`): one payload of periods — the last N ending on today's, starting no earlier than the company's first month with sales, plus every later month with a forecast row, or the range named — each carrying the P&L totals (`actual`, staff cost = the layout's `pl:general:salary` category), the forecast's totals for the same months (`forecast`, null where no month has a row), the Performance P&L's groups, `compare` = performance vs forecast per product group (Sofa / Bedding / Dining / Accessories / Service / Others: the ledger's trading income through the item groups' sales accounts as actual, the Performance P&L's sales and cost, the forecast's lines through the same accounts; a side the period lacks is null), the cost structure by product group (Sofa / Bedding / Accessories / Dining: the Performance cost as Spend, the groups' purchase accounts as Purchase, the stock engine by product category as Closing stock), the Cash Flow's in / out / net and tree, the balance sheet's summary and the six ratios; a period not finished today is `partial`, a future one carries no actuals. Every actual figure is produced by the statements' own builders (`buildPnl`, `buildBalanceSheet`, `buildReceiptsPayments`, `buildPerformance`) over a preloaded window — the ledger and stock replay, the sales orders and their lines, the supplier vouchers' purpose, the money accounts and settings — read once in two parallel waves (`ReportSources` / `RpSources` / `PerfSources`, every read a source), never re-derived; nothing is stored; the Worker caches a payload 60 s per company and query. The page is `/scm/finance-dashboard` (Finance › Forecasting; `FinanceDashboard.tsx` over `DashboardChart.tsx`, hand-drawn SVG at the card's own width, no chart library): Monthly / Quarterly, a from → to month range, the "* not finished" legend and the provisional-stock note; cards Income Statement (six tabs; Actual / % of actual revenue / Forecast / % of forecast revenue / vs Forecast in RM and percentage points, green when better / vs last period), Performance vs forecast by product group (Sales / Gross profit / GP % tabs, All or one group: actual (petrol) and performance (blue, the groups stacked on All) bars side by side, the forecast a red dashed line; rows Actual / Performance / Forecast sales and the gaps in RM and %, Actual vs Performance; gross profit and GP % compare the orders with the forecast only — the ledger's cost of sales is periodic), Cost structure by product group (Spend / Purchase / Closing stock; Show pills — bordered buttons with the group's colour swatch, pressed = on the chart, dimmed when off, Show all / Clear — on Purchase a dashed line = the Forecast P&L's cost of sales through each group's purchase accounts summed over the groups shown, a Forecast purchase row beneath TOTAL), Cash Flow (In / Out / Net, Detail = the tree's top categories), Balance Sheet, Financial Ratios.
- The P&L's stock lines and the Balance Sheet's STOCK read the stock engine as of the date (`accounting-reports.ts` `loadStockLines` over `stockValueByBucketAsOf`): Opening stock = the day before the range, Closing stock = its last day, one line per bucket on the bucket's accounts; the GL's own 330 / 600 / 620 lines are set aside so nothing counts twice; a closing the ledger has not booked for that month-end is flagged `stock.closingProvisional` (the open month, a mid-month date).
- Month-end stock close: nightly 00:05 MYT; on the 1st posts the pair for the month just ended — three closing stocks by warehouse bucket (customer / display / service: the warehouse's `stock_bucket`, else its type; `scm/lib/stock-bucket.ts`) on 330-000x / 620-000x, the next month's opening on 600-000x (`STOCK_BUCKET_ROLES`); owned goods only, consignment-sourced movements never count; other nights heal late documents, re-bucketed warehouses and a month still on the one-account shape by reversing and re-posting; every run logs to `scm.acc_stock_close_runs`, the split in its note.
...
- The P&L and the Performance P&L `expenses` block was seeded with the Cash Flow tree's own expense groups (`backend/scripts/seed-pnl-layout.mjs` over `backend/scripts/lib/pnl-tree.mjs`, Exhibition (Houzs only); Showrooms; Warehouse (Houzs only); Administrative expense → Salary / Rental / Homestay / Marketing / Professional / Office & admin / Transport & logistics / Commission; Finance cost — one rule table with the Cash Flow, `groupOf`; the Cash Flow keeps its own Cost of funds group, the P&L has none); every other block stays the chart's own. Plan by default, `CONFIRM` on apply, a re-run replaces the editor's changes.
