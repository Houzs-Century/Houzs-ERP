...
only, booth on the bar, month/week toggle (`?view=week`, URL is state), prev /
...
| export `GET .../export` | rows Date / Venue / Organizer / Booth / Size — `project_finance` is never read | the same + `totalSales`; the sheet ends with `Brand: X`, `Generated: <time>`, `Confidential` |
