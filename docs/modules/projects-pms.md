...
| event figures | `GET .../events/:eventId` → `{ sizeSqm }` only, reads `projects` alone (owner 2026-09-09 "size not in here. please add also"; the panel shows Size, never money) | `GET .../events/:eventId` → `{ sizeSqm, totalSales }` |
| floorplan PDF | — (no such route; contractors keep Excel only) | `GET .../floorplans?month=YYYY-MM` — the month's events that HAVE a display floorplan, with their files, one statement, logged as `brand_floorplans`; the page's Export MENU ("Event List" = Excel, "Display Floorplan" = PDF) builds the PDF in the browser with jsPDF: one page per floorplan IMAGE (a PDF upload is skipped), event title + dates on top, oldest event first (owner 2026-09-09) |
