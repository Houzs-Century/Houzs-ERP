...
| event figures | `GET .../events/:eventId` → `{ sizeSqm }` only, reads `projects` alone (owner 2026-09-09 "size not in here. please add also"; the panel shows Size and Booth, never money) | `GET .../events/:eventId` → `{ sizeSqm, totalSales }` |
| floorplan PDF | — (no such route; contractors keep Excel only) | `GET .../floorplans?month=YYYY-MM` — the month's events that HAVE a display floorplan, with their files, one statement, logged as `brand_floorplans`; the page's Export MENU ("Event List" = Excel, "Display Floorplan" = PDF) builds the PDF in the browser with jsPDF: one page per floorplan IMAGE (a PDF upload is skipped — on the live database 2026-09-09: 232 JPEG + 49 PNG display floorplans are placed, 14 PDF uploads on 14 events are not), event title + dates on top, oldest event first (owner 2026-09-09) |
| rate cap | both routes: 300 per address per 15 minutes, counting only WRONG or revoked tokens since 2026-09-09 (docs/bugs/0757 — before that every valid request counted and an office locked its own links out); at the cap a valid link is 429 too | same | same |
| export `GET .../export?month=YYYY-MM` — or `?year=YYYY` for a contractor whose picker row has `share_export_scope = 'year'` (mig `20260909T0800_contractor_share_export_scope.sql`; `PATCH /api/projects/contractors/:id`, the "Export whole year" toggle in Project Maintenance; the list route reports it as `exportScope`; owner 2026-09-09: YEN CREATIVE, BAND OF GORILLA, JH CONTRACTOR export the year, DREAM ART the month; month and year together, or a malformed value, is 400) (the month on screen — owner 2026-09-09 "when chose september then click export will export event on september only"; an event touching any day of the month is in; NO month = the whole schedule, because a tab open across the deploy kept asking that way and the export failed for the owner within minutes — a malformed month is 400) | rows Start / End / Venue / State / Organizer / Brand / Type / Booth / Size (Start and End are two bare `YYYY-MM-DD` columns, the shape the ERP's own project CSV export uses, so Excel can filter on either — owner 2026-09-09 "easy to they filter on excel"; Type = `project_event_types.name`; owner 2026-09-09 "tambah state brand type") — `project_finance` is never read | the same + `totalSales`; the sheet ends with `Brand: X`, `Generated: <time>`, `Confidential` |
