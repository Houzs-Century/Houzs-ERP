...
resolves.
Since 2026-09-08 the same file also serves two per-event routes behind the same token gate — `GET .../:token/events/:eventId/floorplan` (the files on the event's `Blank Floorplan` task, else the legacy project-level floorplan) and `.../floorplan/:fileId` (stream, `?download=1` = attachment) — so a contractor can tap an event and view/download ONLY its unfilled floorplan. Full rule and tests: `docs/modules/projects-pms.md`, contractor share links.
