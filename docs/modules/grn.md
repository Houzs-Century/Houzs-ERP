...
Measured on the committed cuts, 2026-09-08: 245 of 1,019 in-scope receipt-to-order
rows have a book location and **all 245 equal the derived value**; 0 of 318
in-scope purchase orders have received lines in more than one location. The other
774 rows are UNKNOWN. Read-only probe: `check-gr-receipt-location.mjs` + Actions
-> **GR receiving location check (read-only)**. Ledger `0677`.
