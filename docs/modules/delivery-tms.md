...
- Board layouts (Columns > Layout): the full board (`DeliveryPlanning.tsx`) ships four team-wide presets — Delivery Date, Delivery Details, SG Order, EM Order (`vendor/scm/lib/deliveryPlanningLayouts.ts`, passed as `layoutPresets`). They lead the picker ahead of the per-company default and the user's own saved layouts; picking one copies its column order + hidden set into the live arrangement. They are EDITABLE team-wide: a layout manager (Owner / Super Admin, the `*` wildcard) uses "Update for everyone (current columns)" to save a company-shared override (`table_layouts` row, `user_id NULL, is_shared 1`, `name` = the preset id, mig 20260925 / D1 161), and "Reset to default" to drop it back to the code seed. Everyone else picks them read-only. The board is a cross-company queue (`SHARED_TABLE_KEYS`), so a shared override is ONE copy for both companies: it is written under the caller's pinned company and forks under other companies are collapsed, and the boot read serves that single copy to everyone. The code seed is the universal fallback, so all users always see the four even before any override. Generic seed support lives in the DataGrid (`LayoutSeed`, `LayoutPresetOption.seed/overridden/readOnly`) + `tableLayouts.ts` (`sharedLayouts`, `saveSharedLayout`/`deleteSharedLayout`) + `routes/tableLayouts.ts` (`PUT/DELETE /:tableKey/shared`, manager-gated). The date/time/crew sub-pages have their own storage keys and do not get presets.
- `delivery_state` and `arrangement_stage` are NO LONGER board columns (owner 2026-09-24): both are still derived server-side and drive the top state tabs and the Pending-Schedule sub-split; the manual `delivery_state` override now lives only on the bulk "Status" action. `em_delivered_date` ("Done Delivery") is also no longer a column (BS Delivery Date is the EM last-mile date) but stays an editable field in the fields drawer.
...
- Board Driver / Lorry / Helper 1-2 cells are type-to-search (`SearchCombo`) matched against the masters by name/plate, keeping an off-list current value selectable; Time Slot is inline free-text (owner 2026-09-26). On an SO row these write the DO crew / DO `time_range` and NEED a live DO — no DO surfaces "create a DO first" (crew) / `no_do_hint` (time) and saves nothing; an ASSR leg's Driver/Lorry still write the trip crew (`type:'assr'`) so the leg consumes capacity. DP rows show n/a; project crew is read-only. Trip No is read-only (the minted `TRIP-YYMM-NNN`).
- `PUT /delivery-orders-mfg/:id/crew` is the only writer of `scm.delivery_order_crew` (driver 1/2, helper 1/2, lorry + snapshots) and syncs driver/vehicle onto the DO header. PARTIAL: a seat named in the body is set (explicit null/'' clears it), a seat the body omits keeps who was on it (`lib/crew-seats.ts` `resolveCrewSeats`) — the board edits one seat at a time; FleetDay sends all five.
