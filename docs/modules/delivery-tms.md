...
- Board layouts (Columns > Layout): the full board (`DeliveryPlanning.tsx`) ships four read-only, team-wide presets — Delivery Date, Delivery Details, SG Order, EM Order (`vendor/scm/lib/deliveryPlanningLayouts.ts`, passed as `layoutPresets`). They lead the picker ahead of the per-company default and the user's own saved layouts; picking one copies its column order + hidden set into the live arrangement. Generic seed support lives in the DataGrid (`LayoutSeed`, `LayoutPresetOption.readOnly`); the date/time/crew sub-pages have their own storage keys and do not get them.
