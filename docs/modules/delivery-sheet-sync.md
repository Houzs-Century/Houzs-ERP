...
- The live `Main.gs` differs from `reference/Main.gs` (the 2026-04 export): the menu is `createMenu('ERP Sync')` with "Pull Latest FROM Houzs ERP" (`manualErpPull`) and "Push Changes TO Houzs ERP" (`manualErpPush`) in place of the AutoCount pull/push items, and `scheduledPull()` / `scheduledPush()` now only log "retired 2026-09-16". That is deliberate: two AutoCount triggers owned by a removed colleague ("Other user") cannot be deleted by anyone left on the project and DID run successfully on 2026-09-16 at 12:09 / 12:18, so their entry points are dead code.
