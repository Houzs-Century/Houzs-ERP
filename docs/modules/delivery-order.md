...
line, with the grid's visible columns and funnels, AutoCount's captions and the book's
spellings. Column design: `docs/line-export-columns.md` §2 with the rulings: **no price or
amount on a line** (the file goes to drivers, 3PLs and customers), **Driver and Vehicle
stay** although empty on every delivery order today, no estimate dates. Default layout:
AutoCount's "LISTING ITEM DETAIL" without its Total, Unit Price and PO DocKey
(`DO_DEFAULT_COLUMNS`, provisional). The list's grid wiring lands in its own PR; the
server, the line and money columns (`pages/scm-v2/so-do-list-columns.tsx`) and the
request (`fetchDoExportRows`, `vendor/scm/lib/so-list-export.ts`) are in place.
...
- **Mobile**: the phone Delivery Orders list (`mobile/MobileModuleList.tsx`) has no
  export. Import is desktop-only by the owner's decision (「手机不需要导入」, 2026-09-15).
