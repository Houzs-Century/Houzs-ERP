...
- Fair picker (`frontend/src/components/FairPicker.tsx`) on all three SO forms: pick place + organizer, no free text; brand comes from the SKUs. A row reads `VENUE — ORGANIZER`; a solo roadshow (event type `solo`) reads `VENUE — SOLO` while the pick still carries the real organizer. Changing the venue on header PATCH resets the fair link to PENDING.
