...
> **`CLOSED` IS RETIRED (owner ruling, 2026-08-21).** He wrote the lifecycle he
> actually runs — Draft, Confirm, In Production, Ready to Ship, Shipped,
> Delivered, Invoice, On Hold, Cancel — and CLOSED is not in it; asked to confirm,
> he narrowed it to this one status: 「照你的流程做，只删 Closed」. It is out of
> `SO_STATUSES` and `SO_STATUS_RANK`, so the manual PATCH refuses it
> (`invalid_status`) and the LIST NO LONGER SHOWS A CLOSED TAB — that tab is
> generated by walking `SO_STATUSES`.
> 
> PROVEN empty before retiring it: company 1 holds 0 sales orders (probe run
> 32487749630) and company 2's own tab counts sum to its total with CLOSED at 0.
> 
> **It stays in two places on purpose.** `SO_TERMINAL_STATES` — a legacy CLOSED
> row must keep being terminal, or it becomes live demand for MRP again — and the
> status-pill LABEL map, so such a row still renders a word. Postgres cannot drop
> an enum label, so the value is permanent whatever the app offers.
