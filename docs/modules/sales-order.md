...
- `CANCELLED` needs an approved cancellation request with a reason (otherwise 403 `cancel_approval_required`), and is refused while a live DO or SI exists. It is FINAL: any move off it is 409 `so_cancelled_final` (`cancel_is_final` when AutoCount holds the order).
...
- `/status` refusals, in order: self-scoped ownership 404, cancel-final 409, version CAS 409 (428 when no version is sent), edit lease 409, then the downstream lock (CANCELLED only).
...
- Client save/proceed pre-flight is ONE shared evaluator, `collectSoSaveProblems` (`vendor/scm/lib/so-save-problems-client.ts`): it returns the FULL blocker list (required fields, completeness, per-line variant/size/fabric, dates, sofa mix, location, payments), and desktop (`SalesOrderNew`) and mobile (`MobileNewSO`) render it in the same `SaveProblemsList` popup the server's 422 uses, so all reasons show at once on both surfaces. Its overlapping gates are pinned to the backend's wording (`so-save-problems-client.parity.test.ts`). `SalesOrderNewGuided` / `SalesOrderNewFromProducts` still call the individual guards.
...
- Fair picker (`frontend/src/components/FairPicker.tsx`) on all three SO forms: pick place + organizer, no free text; brand comes from the SKUs. Changing the venue on header PATCH resets the fair link to PENDING.
...
- The right-click Reopen on a cancelled row is always refused (`so_cancelled_final`). Create a new SO instead, and use Convert to carry the money over.
