...
- `PATCH .../flag-lane` — the row's own lane key, `REQUESTED` lane rows only. The APPROVER saying "this is not mine to approve": a required note (`so_amendments.lane_flag_note`, trimmed, ≤500 chars), never a transition — status, lane and version do not move. The other lane's desk and the requester are told; only the relane script moves the row. The requester is never offered this: they cannot judge the desk.
...
- `frontend/src/pages/scm-v2/WrongApproverFlagButton.tsx` — the approver's wrong-desk flag, one component for the desktop job card and the phone's amendment card.
