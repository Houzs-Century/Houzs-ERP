...
The split was originally generated by `npm run gen:test-projects` into a
committed `backend/tests/generated/test-projects.json` and gated by
...
> **Superseded 2026-08-14 by #2150**, which went one better and removed the
> committed list entirely: `backend/vitest.light.config.mts` now calls
> `classifyTests()` from `backend/scripts/lib/classify-tests.mjs` **at config
> time**, so the split is derived on every run and cannot drift at all. There is
> no `backend/tests/generated/test-projects.json` any more. Measured 2026-08-14:
> **light 236 / workers 44**.
