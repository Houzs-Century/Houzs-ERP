...
**Why some never self-heal (the state on 2026-07-24 — §7 closed this on 2026-07-29, and the fix is in the tree today: `reconcileUncostedAfterIn` is called from 14 files):** `fn_reconcile_uncosted_out` is invoked from EXACTLY ONE place — the GRN post handler (`grns.ts:493`, via `reconcileUncostedOuts` in `oversell-retrocost.ts`). Verified by exhaustive grep: no other caller, and no scheduled sweep exists. But lots are opened by MANY other stock-IN paths that never call any reconcile:
