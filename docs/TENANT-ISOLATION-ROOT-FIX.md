...
| 2 | **Cross-company impersonation**: `POST /users/:id/impersonate` resolves the target by id with no company check, while `PUT /users/:id/companies` in the same file constrains grants to what the actor holds. | Impersonation grants the actor another company's session. Under RLS it becomes the supported way to cross the boundary — which may be right, but it should be a decision. |
| 3 | **Presence**: `/api/presence` lists every user with `last_path`, cached under the key `scope=all` — one entry shared by every caller. | Scoping the query alone would not help; the cache key must carry the company. |
