...
| --- | --- | --- |
| 1 | ~~**Zero-grant users**: `companyContext.ts` hands a user with no `user_companies` row EVERY company, deliberately. Flip to fail-closed?~~ — **DECIDED + IMPLEMENTED 2026-08-20.** | **Flip to fail-CLOSED.** A resolved multi-company user with ZERO `user_companies` grants now gets `allowedCompanyIds = []` (restricted-to-nothing) instead of every company — see `backend/src/middleware/companyContext.ts` (the `else` on the `granted.length > 0` branch) and `docs/TENANT-ISOLATION-ROOT-FIX.md`'s sibling ledger entry in `BUG-HISTORY.md`. Only the RESOLVED, confirmed-zero-grant case flips: a grant-read error still fails OPEN (never lock out on a blip), single-company installs never narrow, and the cold-start branch is untouched. **Safe now because the `Tenant isolation check (read-only)` audit found 0 users with zero grants** — nobody is locked out; it is a safety default for the future. This also unblocks RLS: under RLS `app.company_id` must be a single value, and a user granted "all" had none. Pinned by `backend/tests/companyContextZeroGrantFailClosed.test.ts`. |
