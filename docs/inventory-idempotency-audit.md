...
| 2 | **GRN confirm / post** — `grns.ts` `postGrnAndRollup` | IN | ~~**G-partial only**~~ → **FIXED 2026-07-29 (§6a): G3 atomic CAS `.eq('status', preStatus)` + G2 movement-existence guard + CANCELLED/CLOSED refused at the chokepoint.** Still **no G1 index** — and deliberately so: a unique index would break the line-edit / line-add / cancel / relocate writes (§6a) | Editing a POSTED GRN DOES write delta movements under `source_doc_type='GRN'` (line-add, line-edit qty delta, warehouse relocate) — this is why G1 is unsafe here | ~~MEDIUM–HIGH~~ → **SAFE against the concurrent-confirm double-post.** The CAS serialises the transition before any stock is written; the existence guard covers non-confirm callers. See BUG-HISTORY 2026-07-29. |
...
1. ~~**GRN confirm has NO DB double-post net (new, MEDIUM–HIGH).**~~ **CLOSED 2026-07-29 at the app layer — see §6a.** Unlike DO/DR, `postGrnAndRollup` relied solely on a route-level status read plus a non-atomic `.neq('status','CLOSED')` flip, so a concurrent confirm of a DRAFT GRN could write the IN movements twice. It now uses an atomic compare-and-swap plus a movement-existence guard. The DB index originally recommended (R1) was **refuted** — GRN, PURCHASE_RETURN, STOCK_TRANSFER and STOCK_TAKE are all legitimately multi-row-per-bucket, so a UNIQUE index would break real writes (§6a). `PURCHASE_RETURN` still relies on its route-level early return. Logged in `BUG-HISTORY.md`.
...
### 6a. UPDATE 2026-07-29 — R2 + R3 SHIPPED; **R1 REFUTED** (do not attempt it)

**R2 and R3 are implemented** (`fix/grn-double-post-index`): the `postGrnAndRollup` flip is now an atomic compare-and-swap on the status the call observed (`.eq('status', preStatus)`, expected value READ rather than hard-coded to `'DRAFT'` because the create-as-posted paths call the chokepoint with the row already `'POSTED'`), CANCELLED/CLOSED are refused at the chokepoint, and a movement-existence guard mirroring `deductInventoryForDo` guard #1 skips the IN write when this GRN already has `GRN`/`IN` rows. `postGrnHandler` now returns **409** on a lost race instead of a blanket 500.

**R1 (the per-document partial UNIQUE index) was traced and must NOT be built** — for GRN *or* for the other three types the detector labels "single-post". Every one of them is legitimately **multi-row-per-bucket** under its own `source_doc_type`, so the index would reject real writes:

| Type | Why an index is UNSAFE | Verdict |
|---|---|---|
| **GRN** | 7 write sites under `'GRN'`. The **line-edit qty-delta IN** (`grns.ts`, "GRN line qty edited — receiving delta") shares the FULL key with the post IN — same warehouse, product, variant, batch AND `movement_type`. Also line-add IN on a POSTED GRN, cancel OUT, warehouse-relocate OUT+IN. Unlike DO/DR, GRN does **not** route its reversals onto `ADJUSTMENT`. `grn_items` has no uniqueness, so one GRN can carry two lines of the same material from one PO → two post INs with an identical key. | **EXCLUDE** |
| **PURCHASE_RETURN** | `writePrLineDelta` writes further OUT rows ("PR line added/increased") with an identical key to the post OUT. | **EXCLUDE** |
| **STOCK_TRANSFER** | It is the deliberate escape-hatch type for the consignment paired INs *because* it has no unique index (`consignment-loaner.ts`: "under STOCK_TRANSFER (NOT the CS_* type) so it never collides with the OUT on the CS_* unique index"). Five modules write it under consignment doc ids using the ledger-delta pattern. The real transfer path is `fn_stock_transfer_apply`, prod-only and not inspectable here. | **EXCLUDE** |
| **STOCK_TAKE** | The reversal writes `ADJUSTMENT` rows with an identical key to the forward post (also `ADJUSTMENT`). Already the strongest-guarded path (atomic CAS on `.eq('status','OPEN')`) — an index adds no protection and breaks the reversal. | **EXCLUDE** |
| **ADJUSTMENT** | The movement row IS the document; no `source_doc_id` to key on. | **EXCLUDE** |
| **PC_RECEIVE / PC_RETURN** | G4 ledger-delta by construction — repeated delta rows per bucket under one `source_doc_id` is the design. | **EXCLUDE** |

**So the set of types with a DB-level double-post net is UNCHANGED**: only `DO`, `DR` (prod-only DDL) and `CS_DO`, `CS_DR` (migration 0153). GRN's protection is now app-level (G2 + G3) rather than G1 — which is what the concurrent-confirm race actually required, since the CAS serialises the transition *before* any stock is written.

> **Why the "0 duplicates in prod" measurement did not settle it.** `check-duplicate-movements.mjs`'s `SINGLE_POST_TYPES` set is a **detection heuristic** for surfacing suspicious groups to a human, not proof of an enforceable invariant. Zero measured duplicates means the shape has not occurred *yet*; a UNIQUE index converts that into "can never occur", which is only safe once the write paths have been read. The measurement also only covers the detector's full key (incl. `warehouse_id`, `batch_no`, `movement_type`) — it is silent about any narrower index, which could collide on rows the detector counted as distinct.
