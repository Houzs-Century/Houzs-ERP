...
| # | finding | status |
|---|---|---|
| F1 | `scm.pos_carts` keyed `staff_id` alone — switching company destroyed the other company's cart, silently. | **DONE** — mig 0289 (0284 when this was written; that number is now `0284_retire_consignment_proceeded_at.sql`, and 0289's own COMMENT ON TABLE still says "since mig 0284" because the migration is APPLIED and its body cannot be edited) re-keys `(staff_id, company_id)`; 9 pg tests **written but NOT RUN** (no Docker, no `TEST_DATABASE_URL` here — CI is the gate) |
...
**Honest count at the time: 20 unscoped writes. CLOSED SINCE** — `check-company-scope --strict` runs at `ci.yml:98` and reports **0 WRITE** findings over 1018 handlers. The rest of this paragraph is the reasoning as it stood; the CI step WAS report-only
until they reached zero — gating on a backlog fails every PR and gets the gate
...
| K4.2 | `backend/src/services/salesTeam.ts:228` `autoBackfillSalesReps` | self-heal for missing `sales_reps` rows | `BUG-HISTORY.md` line 7948 already records it as never called. The open question is whether to **wire** it, not whether to delete it — deleting removes the recovery tool the bug entry points at. Owner call |
| K4.3 | `backend/src/services/printTracker.ts` (whole file, 189 lines; `renderStageTrackerHtml`, `STAGE_TRACKER_CSS`) | print stage tracker | **A deliberate keep is already on record.** `BUG-HISTORY.md` line 6337: "Deliberately NOT done ... it has no importer at this commit ... rewriting a print artifact's stage derivation is a behaviour change that does not belong in a delete-dead-code commit." Also labelled in `docs/modules/service-case.md:101`. Deleting it would silently reverse a written decision |
