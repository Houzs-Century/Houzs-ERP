...
| 2 | Should `apply-sofa-compartment-corrections.mjs` keep its DELETE branch? | **No - change it to set `cancelled = true`.** The refusal guard already protects referenced rows; this closes the unreferenced case the rule now also covers. Until it is changed, do not re-run the script on any build whose piece count shrinks. |
