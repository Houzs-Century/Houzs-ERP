...
**Root cause (traced).** Two importers, and the display units fall through the
gap between them. `import-ac-stock-balance.mjs` excludes every SOFA-category row
...
**Fix.** Section 6b of `backend/scripts/import-ac-sofa-stock.mjs` opens them the
