...
checker against production (runs 34099375384 and 34099753029) rather than by
reading it.
...
1. `isSofaCode` is a bare `/SOFA/i` substring test, so an ACCESSORY whose NAME
   ...
   no digits, so both sides yielded `null`. The branch then read
...
2. The model was not folded through `SOFA_MODEL_ALIAS`. The floor writes the
   ...
   5536/9058, 5537/8030, 5540/8030); ten other scripts in `backend/scripts`
   fold it before comparing and this checker was the only one that did not, so
...
The 9 real sofa-model disagreements that remain are named in the PR and are an
owner decision, not a checker defect.
