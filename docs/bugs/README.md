...
- **Two owners in one jsonb value.** `scm.mfg_products.seat_height_prices` holds `priceSen` (COST, ours) beside `sellingPriceSen` (RETAIL, authored only from 2990's POS SKU Master); nothing in the column's type keeps them apart, so a writer that assigns the whole array deletes the other team's money with no error, visible only to them, days later — a cost edit wiped it in June, then auto-derive erased 193 retail prices across 82 SKUs over four days in September → merge per (height, tier), carry an omitted key forward, re-append slots the payload dropped, and enforce it in the database where the owner is a different company → `backend/scripts/check-multi-owner-jsonb.mjs`, `backend/tests/multiOwnerJsonbScan.test.mjs`, `backend/src/scm/lib/auto-derive-cost.test.ts`, `backend/src/db/migrations-pg/20260920T1300_scm_company2_retail_price_lock.sql`.
- **Another system's value in our column.** A column this backend writes but does not OWN — company 2's retail prices, authored only from 2990's POS — has no signal a guard can read when it is a plain scalar (`sell_price_sen`, `pwp_price_sen`): an UPDATE that sets it wrong is spelled exactly like one that means to, and the loss shows up on the other team's screen, days later → lock what carries intent (key presence in a jsonb slot), and WATCH the rest against the audit trail the damaging writer does not touch, daily, alarming only on a finding → `backend/scripts/check-2990-retail-price-sentinel.mjs`, `backend/tests-pg/retailPriceSentinel.pg.test.ts`, `.github/workflows/check-2990-retail-price-sentinel.yml`.
