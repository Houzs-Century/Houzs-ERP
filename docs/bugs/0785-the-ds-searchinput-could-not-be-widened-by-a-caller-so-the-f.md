...
**Fix.** `SearchInput` gained a `widthClassName` prop defaulting to `"w-72"`, applied in the field's own `cn` slot IN PLACE OF the hardcoded width. Every existing caller is byte-for-byte unchanged (default `w-72`); the Roles rail passes `widthClassName="w-full"` for a fluid field with no class collision. Verified in an isolated Vite preview (real Tailwind) that the rail search fills its column; `tsc -b` stays green and no existing `SearchInput` caller changed.
