...
| `sales-order.md` | partial (11 of ~24 sections) | Claude, #2231 + #2233 + #PENDING | 10 correct in full; 2 findings, both in earlier passes. NEW THIS PASS, all three CORRECT: selling-price authoring (the trust gate is by SESSION not role — `isPosTabletCaller` reads `sessionOrigin`; the flag reaches all THREE paths the guide names, create/add-line/patch, verified at 4094/7959/8499; drift-reject fires only for POS; `canEditPrice` is byte-identical on SoLineCard and MobileNewSO; `isHatchSales` is `sales || super_admin`), delivery fee (mig 0214 RPC really does advisory-lock + delete/insert/stamp, and it has exactly ONE caller in the routes; the bail rule is genuinely "no SVC-DELIVERY lines AND no header fee"), and the by-code company-scope table (all 12 named helpers take a companyId — checked mechanically). NOT READ: address block, list handler, amendments, line photos, caching, the 8-name Processing-Date column registry. |
