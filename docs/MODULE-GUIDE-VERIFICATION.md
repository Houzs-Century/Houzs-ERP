...
| `change-log.md` | not verified | — | written 2026-09-08 with the module; not yet read back by anyone but its author, which is what `not verified` means |
| `chat-callback.md` | written with the code (#3211) | Claude, #3211 | NEW guide, 2026-09-02 — written in the same PR as the endpoint it describes, because the working-agreement check had no guide to point at for the `/api/chat-callback` mount. Every claim was read at its file on that date: the pre-auth mount order in `index.ts`, the guard's constant-time compare / 250ms delay / per-IP limiter, the three-state company contract and the identical 404 for "no such SO" and "not yours", the sanitised `callback_id` and the company+source-scoped duplicate probe, and the `source='delivery-planning'` predicate added to `delivery-messages./statuses` in the same commit. **Written with the code, so it carries no INDEPENDENT verification** — one reader's read of their own route, not a second reader checking the first. NOT ESTABLISHED: any live behaviour whatsoever. `CHAT_CALLBACK_KEY` is unset, the endpoint 401s every request, and it has never run against a real callback — §9 is a procedure, not a report. |
...
| `goods-receipt.md` | written with the code (#3519) | Claude, #3519 | NEW guide, 2026-09-10 — created because the working-agreement check had no guide to point at when the zero-cost receipt guard changed. It documents ONE surface only: the owner's 2026-09-10 ruling that a receipt saves without a price, and the `zero_cost_ack = true AND zero_cost_ack_by IS NULL` marker that keeps the zero findable. **Everything else about goods receipts is NOT in it** — this is a stub with one true section, not a module guide. |
| `memos.md` | written with the code (memo register PR), re-shaped the same day (register in Announcements PR) | Claude, 2026-09-09 | NEW guide, 2026-09-09 — written alongside the register it describes (mig 20260909T0500, `routes/memos.ts`, `pages/announcements/RegisterView.tsx`) |
| `document-refs.md` | written with the code (#3015) | Claude, #3015 | NEW guide, 2026-09-06 — written alongside the registry it describes (mig 20260906T1417, `services/documentRefs.ts`, `routes/documentRefs.ts`), so it carries no INDEPENDENT verification: the number shape, the series key, the counter call (`scm.next_doc_no_n`) and the fallback path were read at their files on that date and are pinned by `tests/documentRefs.test.ts` on the D1 mirror (fallback path only). NOT verified: the counter path against a deployed Worker — no consumer has minted yet. |
| `delivery-order.md` | not verified | — | — |
| `document-cancel-approval.md` | not verified | — | written 2026-09-08 with the feature |
| `document-status-vocabulary.md` | not verified | — | — |
| `delivery-rate-card.md` | not verified | — | — |
| `delivery-return.md` | not verified | — | — |
| `delivery-sheet-sync.md` | written with the code (#3955) | Claude, #3955 | NEW guide, 2026-09-15 — written in the same PR as the two `/api/delivery-sheet` routes it describes. Every claim was read at its file on that date: the pre-auth mount order in `index.ts`, the `SHEET_SYNC_KEY`-only guard and the refusal when the companies master has no HOUZS row, the feed SQL (company predicate, DRAFT/CANCELLED exclusion, `GREATEST` over header/payment/DO for `LastModified`, cancelled DOs and POs dropped) and the write leg (found by `linked_ac_docno` OR `doc_no`; a blank date keeps, an absent remark keeps). The column rulings are the owner's own words from the 2026-09-15 session. **Written with the code, so it carries no INDEPENDENT verification.** PROVEN, read-only on production: the feed SELECT runs (25 ms over 2,958 orders). NOT ESTABLISHED: any live sheet behaviour — `reference/ERPDeliverySync.gs` has not yet been pasted into the live Apps Script project, so §4 is a procedure, not a report. |
...
| `so-amendment.md` | written with the code (2026-09-09) | Claude, `feat/amendment-pending-badge-0909` | NEW guide, 2026-09-09 — the gap the working-agreement gate had been WARNing about: the SO amendment module is the larger of the two (lanes, splitting, PO follow-ups, a notification audience) and had no guide, while its PO sibling did. Written from source in the same session that changed the module three times, so the workflow half was read at its files: the lane table and the state machine at `shared/amendment-lane.ts`, the gate keys at each `PATCH` handler in `routes/so-amendments.ts`, the split at the create route in `mfg-sales-orders.ts`, the notice audience at `services/amendmentNotify.ts` (and proven by the prod rows behind `docs/bugs/0743`). §5's claims are pinned by `soAmendmentPendingCount.test.ts`. NOT independently verified: §1's service-SKU routing rule and the legacy supplier-confirm chain, both summarised from source comments rather than traced through a run; §6 is a pointer table, not claims. |
...
| `venture-portal-feed.md` | written with the code | Claude, 2026-09-12 | NEW guide, 2026-09-12 — written alongside the feed it describes (mig `20260912T1800`, `scm/lib/venture-portal-outbox.ts`, `scm/routes/venture-portal-feed.ts`, the two page surfaces), so it carries no INDEPENDENT verification: it is one reader's read of their own code, not a second reader checking the first. **What IS established, against PRODUCTION (`anogrigyjbduyzclzjgn`) on 2026-09-12 via the Supabase MCP** — that `pg_net` and `pg_cron` are NOT installed (the finding §2 is built on, and the contract's own open point); every column the payload reads, with its type, on the payment-totals view, `mfg_sales_order_items`, `mfg_sales_order_payments` and `scm.staff`; that `scm.sync_config` exists and is EMPTY and `scm.app_config` has the shape the migration writes to; and that 493 sales orders are in the §10 backfill's scope. The statement splitting in §3 was verified by running `backend/scripts/lib/split-sql.mjs` over the migration (20 statements, every dollar-quoted body intact). The response taxonomy in §6 and the three off switches in §0 are pinned by `backend/src/scm/lib/venture-portal-outbox.test.ts`. **The payload builder in §4 IS established, against production, read-only.** Its whole body was run there as an anonymous `DO` block on 2026-09-12, and a query assembled one real order's delivery: `HC-SO-2609-063`, 8,506 bytes, 4 items, 1 payment, salesperson resolved through the `::uuid` cast and the `scm.staff` join, `items[0].line_cost_sen` present, `header.balance_sen_live` present, `header.phone` and `header.approval_code` absent — 0 of the 24 stripped keys survive and all 19 the portal reads do. The Supabase MCP connection is READ-ONLY, which is both why this was safe and why it is the LIMIT of what can be established from here: what ran was the expression and the body, never the `CREATE FUNCTION` wrapping them. **NOT ESTABLISHED — the DDL, and everything about this feed RUNNING.** The migration has been applied to no database, so the table, the three triggers, the two function wrappers, the grant block and the seed have never executed anywhere, no trigger has ever fired, and `vp_build_payloads` has never returned a row. A syntax error in any of them fails the deploy's `pg-migrate` step and blocks every later migration until fixed — so the Deploy run must be READ, not assumed. No delivery has been attempted against the live portal — but the receiver was probed on 2026-09-12 and answered `503 {"ok":false,"error":"erp sync not configured"}`, which places it exactly where the contract says an unset `ERP_SYNC_SECRET` leaves it; every other statement about how it answers comes from its contract, not from having called it. §10's backfill timing is arithmetic over a measured row count, not a run. |
