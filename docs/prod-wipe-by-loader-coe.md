...
- `backend/scripts/scale-target-guard.mjs` + `backend/tests/scaleTargetGuard.node.mjs` — the allow-list guard pattern this file should adopt. (Corrected 2026-08-14: cited here and at `:103` as `scaleTargetGuard.test.mjs`, which does not exist. The `.node.mjs` suffix is load-bearing — it is how the runner picks the file up.)
