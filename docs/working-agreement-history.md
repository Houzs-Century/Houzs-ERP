# Houzs ERP — Working agreement: history and reasoning

`CLAUDE.md` is auto-loaded into every session and every subagent, so since
2026-09-15 it carries only the rules, one bullet each. This file keeps what
bought each rule — the incidents, the owner's words, the CORRECTED boxes, the
worked examples and run ids — **verbatim**, under the same section headings,
exactly as `CLAUDE.md` stood at `b41060523` (1,368 lines). One thing was
removed: a duplicated description of the `main-protection` ruleset (two
paragraphs that restated the ruleset's four rules a second time).

Read this when a rule in `CLAUDE.md` looks wrong, arbitrary or over-strict —
the reason is here. **"This file" in the text below means `CLAUDE.md` as it was
then**, and every count and date is as of when it was written: re-run, do not
recall. `docs/working-agreement-rule-inventory.md` maps each rule id to its
section. Nothing here is a rule that `CLAUDE.md` lacks; if you find one, that
is a defect — add the bullet to `CLAUDE.md`.
