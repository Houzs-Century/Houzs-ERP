...
## Plans in force
- One table component (owner 2026-09-25): every list renders `components/DataTable.tsx`; the vendored SCM `DataGrid` is retired. Order: (1) DataTable gains what DataGrid pages use — client search, double-click, row style, plain xlsx export, date/number filters, default sort, disabled/checkbox-only selection, compact embedded mode, drag-to-group multi-level group-by (owner chose full parity 2026-09-25 although 1 of 40 saved dg layouts groups), `dg:` saved-layout carry-over; (2) move the 37 DataGrid files in batches (easy lists, "from X" pickers, detail listings, consignment, Delivery Planning board last), delete DataGrid; (3) hand-rolled list tables; (4) then the table features together: resize moves one column only (parked on branch `feat/datatable-unify-p1`), wrap text, real freeze, English Title Case headers, footer totals (filtered / selected), double-click auto-fit, Ctrl+C to Excel, export = what is on screen, column filters in the URL, uniform number/date/empty/status format, fixed overdue/owing highlight rules, reset-to-company-default, header tooltips.
