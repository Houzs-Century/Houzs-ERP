...
## Waiting on owner
- 2990 POS combos now live in `scm.pos_sofa_combos`; the Venture Portal catalogue for company 2 still reads `sofa_combo_pricing`, so a combo created on the POS after the split does not reach the portal — decide whether it should. Then drop `scm.bak_combo_restore_20260926` and `scm.bak_combo_houzs_p1_moved_20260926` — 2026-09-26
...
## Plans in force
- One table component (owner 2026-09-25): every list renders `components/DataTable.tsx`; the vendored SCM `DataGrid` is gone (DataGrid pages render through `components/DataGridCompat.tsx`). Left: (3) hand-rolled list tables onto DataTable; (4) then the table features together: resize moves one column only (parked on branch `feat/datatable-unify-p1`), wrap text, real freeze, English Title Case headers, footer totals (filtered / selected), double-click auto-fit, Ctrl+C to Excel, export = what is on screen, column filters in the URL, uniform number/date/empty/status format, fixed overdue/owing highlight rules, reset-to-company-default, header tooltips.
