...
## Waiting on owner
- Roles & Permissions part B follow-ups: (1) `pmsAccess.ts` director / PIC / purchasing regexes and `projectGates.ts` warehouse-crew prefix still key on the Title's NAME — move onto `position_policy`; (2) phone has no Titles editor (desktop-only like the Actions / SCM matrices); (3) PG WH Assistant seeded `full` (was fail-open) — owner to say if it should be restricted / storekeeper; (4) 7 Titles with no active member (Sales Person, Operation Manager, Procurement/Purchasing, Helper, Service Admin, Calendar Viewer, PG WH Assistant) and 4 roles with no holder (Service Admin, Calendar Viewer, Managing Director, PG WH Assistant) — keep or delete — 2026-09-16
