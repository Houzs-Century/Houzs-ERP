...
// has been back FOUR times. Each trip is now a row here (round_no 1..N, added
// freely). This table is the SOURCE OF TRUTH for the trip dates; the two columns
// on assr_cases MIRROR the current (highest round_no) row so the Delivery board
// and the HC Delivery sheet keep reading the trip in progress. Both surfaces
// render this list via the shared helper vendor/scm/lib/assr/returns.ts.
