...
export async function convertSosToPosCore(c: PoConvertContext): Promise<PoConvertOutcome> {
  ...
  for (const bucket of byGroup.values()) {
    ...
    const headerPayload = {
      ...
      tax_centi: 0,
      total_centi: subtotal,
      /* The PO's PROVENANCE, and for these POs the only record of it — a bulk
         MRP raise writes no per-line so_item_id, so this note is what the
         relationship map, the coverage endpoint and the printed PO all read
         to answer "which orders was this bought for".

         Generated by the #2370 label rule since 2026-08-18, so the note says
         the same sentence as the button that raised it ("Transfer from Sales
         Order") instead of a private sixteenth wording. Every reader still
         accepts the pre-rename "From SOs:" / "From SO:" spellings — see
         shared/transfer-vocabulary.ts. Do NOT inline this string. */
      notes: provenanceNote('so', [...bucket.soDocNos]),
      created_by: user.id,
