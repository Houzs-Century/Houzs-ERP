...
grns.post('/', async (c) => {
     ...
     Refused only when the named PO already orders that material. */
  {
    ...
    if (unlinked.length > 0) return refuseWithoutWriting(c, unlinkedPoLinesResponse(unlinked), 409);
  }
...
export const createGrnFromPosHandler = async (c: Context<{ Bindings: Env; Variables: Variables }>) => {
  ...
  if (itemList.length === 0) return refuseWithoutWriting(c, { error: 'nothing_outstanding', message: 'All PO items are already fully received' }, 400);
