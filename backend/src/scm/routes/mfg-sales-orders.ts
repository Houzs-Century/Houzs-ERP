...
mfgSalesOrders.post('/:docNo/amendments', async (c) => {
  ...
  /* ── Two-lane split (owner rework 2026-07-27) ─────────────────────────────
     ...
     off the line's identity (item code + item_group), resolved SERVER-SIDE from
     the order (an ADD has no persisted line, so its requested new_item_code is
     used). item_group is what routes a BARE-code service line (DISPOSE / STORAGE
     / TRANSPORTATION CHARGES — item_group='service', no SVC- prefix) to
     Logistics instead of Purchasing (owner 2026-09-11, docs/bugs). */
  ...
  const split = splitAmendmentByLane(
    ...
    (l) => (l.salesOrderItemId ? itemGroupById.get(l.salesOrderItemId) : null),
  );
