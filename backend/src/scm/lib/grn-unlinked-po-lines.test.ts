...
describe('poMaterialCodesOf', () => {
  it('returns every material the named PO orders, upper-cased and trimmed', () => {
    ...
    return expect(poMaterialCodesOf(sbWith([' foam-40d ', 'Fabric-A']), PO_ID)).resolves.toEqual(
      new Set(['FOAM-40D', 'FABRIC-A']),
    );
  });

  it('is empty when the GRN names no PO — the legitimate free-receipt case', async () => {
    for (const id of [null, undefined, '', '   ']) {
      expect(await poMaterialCodesOf(sbWith(['FOAM-40D']), id)).toEqual(new Set());
    }
  });
});
...
describe('findUnlinkedPoLines', () => {
  it('REFUSES an unlinked line receiving a material the named PO already orders', async () => {
    ...
    const out = await findUnlinkedPoLines(sbWith(['FOAM-40D']), PO_ID, 'HC-PO-2608-001', [line('0', 'FOAM-40D', 12)]);
    expect(out).toEqual([
  ...
  it('REFUSES across a case / whitespace difference on EITHER side', async () => {
    ...
    const out = await findUnlinkedPoLines(sbWith([' foam-40d ']), PO_ID, 'HC-PO-2608-001', [
      line('0', '  FOAM-40D', 12),
    ]);
  ...
  it('ALLOWS an unlinked line for a material the PO does not order', async () => {
    ...
    const out = await findUnlinkedPoLines(sbWith(['FOAM-40D']), PO_ID, 'HC-PO-2608-001', [line('0', 'SAMPLE-SWATCH', 1)]);
    expect(out).toEqual([]);
  });
  ...
  it('ALLOWS everything when the GRN header names no PO', async () => {
    for (const id of [null, undefined, '', '   ']) {
      expect(await findUnlinkedPoLines(sbWith(['FOAM-40D']), id, 'HC-PO-2608-001', [line('0', 'FOAM-40D', 12)])).toEqual([]);
    }
  });
  ...
  it('queries NOTHING when every line is already linked', async () => {
    ...
    const out = await findUnlinkedPoLines(sb, PO_ID, null, [line('0', 'FOAM-40D', 12, 'poi-0')]);
    expect(out).toEqual([]);
  ...
  it('still REFUSES when the PO row carries no po_number — the id is the last resort', async () => {
    ...
    const noNumber = await findUnlinkedPoLines(sbWith(['FOAM-40D'], null), PO_ID, null, [line('0', 'FOAM-40D', 12)]);
    ...
    expect(await findUnlinkedPoLines(noHeaderRow, PO_ID, null, [line('0', 'FOAM-40D', 12)])).toEqual([
  ...
  it('uses the caller-supplied PO number without re-reading the header', async () => {
    ...
    const out = await findUnlinkedPoLines(sb, PO_ID, 'HC-PO-2608-009', [line('0', 'foam-40d', 12)]);
    expect(out[0]?.poNumber).toBe('HC-PO-2608-009');
  });

  it('reports every offending line, and only those', async () => {
    const out = await findUnlinkedPoLines(sbWith(['FOAM-40D', 'FABRIC-A', 'LEG-SET']), PO_ID, 'HC-PO-2608-001', [
