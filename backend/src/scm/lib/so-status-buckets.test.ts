...
import { SO_STATUS_BUCKETS, soStatusesForTab } from "./so-status-buckets";

describe("SO_STATUS_BUCKETS", () => {
  ...
  test("SHIPPED is reachable from a tab and is in exactly one", () => {
    const owners = Object.entries(SO_STATUS_BUCKETS)
  ...
  test("there is no Shipped tab left to select", () => {
    expect(SO_STATUS_BUCKETS.SHIPPED).toBeUndefined();
  });
  ...
  test("every key is upper case, matching what the client sends", () => {
    for (const tab of Object.keys(SO_STATUS_BUCKETS)) expect(tab).toBe(tab.toUpperCase());
  });
