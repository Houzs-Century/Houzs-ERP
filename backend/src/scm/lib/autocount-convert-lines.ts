...
export const DOWNSTREAM: Record<'DO' | 'GR' | 'IV' | 'PI', AcDownstreamSpec> = {
  ...
  GR: {
    ...
    itemCols: 'id, material_code, item_group, description, description2, qty_accepted, unit_price_sen, variants, linked_ac_dtlkey, created_at',
  ...
  PI: {
    ...
    itemCols: 'id, material_code, item_group, description, description2, qty, unit_price_sen, variants, linked_ac_dtlkey, created_at',
    docNoOf: (h) => String(h.invoice_number ?? h.id ?? ''),
    line: soLine,
    header: (h) => present({
