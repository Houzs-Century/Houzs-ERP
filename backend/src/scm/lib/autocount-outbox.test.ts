...
describe('the three fields the extract carries and the write-back did not send', () => {
  ...
  /* ── THE DELIVERY ADDRESS, which the EDIT never sent ─────────────────────
     CreateSo falls back per line (DeliverAddr1 = Or(DeliverAddr1, InvAddr1)), so
     a document created WITH an address gets both copies. /edit's header loop is
     ContainsKey-gated and soEditHeader only ever emitted InvAddr*, so an address
     added or changed AFTER the create updated one copy and left the other.

     Measured on the live book 2026-08-16: HC-SO-2608-002, address typed in by an
     edit, carries InvAddr1 `dsdsd` / InvAddr3 `05200 Alor Setar` against three
     EMPTY DeliverAddr lines. HC-SO-2608-003, same shape but addressed at create,
     has both. Inistate — the connector this replaces — writes them identical. */
  describe('an EDIT carries the delivery address, not only the invoice one', () => {
    ...
    test('clearing the address nulls both copies', async () => {
      ...
      await enqueueEdit(client(sb), {
        companyId: 1, docType: 'SO', docNo: 'HC-SO-B', touchedFields: ['address1'],
      });
      const h = header(sb);
      for (const n of [1, 2, 3, 4]) {
