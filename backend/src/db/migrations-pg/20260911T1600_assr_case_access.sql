-- 20260911T1600_assr_case_access.sql
-- REVERSAL: DROP TABLE IF EXISTS public.assr_case_access;
--
...
-- subtree, or the case's ERP-native SO names a subtree member as salesperson
-- (see assrVisibilityPredicateSql, services/assrVisibility.ts). That gives a
-- case ONE salesperson + TWO assignee slots. Ops needs to keep the SALESPERSON
-- on the original rep (sales attribution) while letting an OPEN-ENDED set of
-- other staff reach the case - e.g. Kingsley stays the salesperson while
-- Stanley, Shawn (and more later) work it. Two slots cannot express "four+
-- people", so this table is the Nth-person reach. It NEVER touches sales_agent.
--
-- SHAPE. One row per (case, granted user). The grant is a preference, not a
-- dependency; the same additive OR-branch the id-scope already uses reads it as
-- a 6th arm inside assrVisibilityPredicateSql:
--   OR EXISTS (SELECT 1 FROM assr_case_access acc
--              WHERE acc.assr_id = <case>.id AND acc.user_id IN (<caller subtree ids>))
-- so a granted user AND their upline reach the case, exactly like assigned_to.
-- ONE predicate = one place the rule lives; every reader (list, detail, print,
-- aggregates, board) resolves through it.
--
...
-- splits on ';\n'). Timestamp is the next free one over main at write time;
-- re-pick it at MERGE time if a later migration lands first.
