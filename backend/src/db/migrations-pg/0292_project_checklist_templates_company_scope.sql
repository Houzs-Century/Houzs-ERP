-- 0292 — project checklist TEMPLATES become per-company: record the contract in
--
...
COMMENT ON TABLE public.project_checklist_templates IS 'Master checklist templates cloned into every new project. PER-COMPANY since mig 0292 (owner decision 2026-08-13): company_id names the owning company AND is the READ-SCOPE predicate - routes/projects.ts filters every template list and every by-id body read with activeCompanySql(c), so one company cannot see or edit the other company template master. Column itself was added by mig 0093, which described it as a schema-parity backstop NOT used to scope reads; 0292 reverses that for the three template tables only. Existing rows backfilled to HOUZS (all pre-split templates were authored by Houzs); 2990 starts empty and builds its own.';

COMMENT ON TABLE public.project_checklist_template_sections IS 'Sections of a checklist template. PER-COMPANY since mig 0292 - company_id is stamped on insert from the active company AND is the read/write-scope predicate. Note the parent template_id is ALSO re-checked against the active company before any create/reorder, because a stamp is not a predicate: stamping company_id on a new section says nothing about whose template it was hung under. Existing rows backfilled to HOUZS.';

COMMENT ON TABLE public.project_checklist_template_items IS 'Task rows of a checklist template, cloned into project_checklist on project create. PER-COMPANY since mig 0292 - company_id is stamped on insert from the active company AND scopes every UPDATE/DELETE by item id, so a cross-company id reaches nothing and answers 404 rather than silently mutating. The parent template_id is re-checked against the active company on create and reorder. Existing rows backfilled to HOUZS.';
