...
--
-- REVERSAL: per table (scm.purchase_invoices, scm.sales_invoices):
--           DROP INDEX IF EXISTS scm.<table>_migrated_ac_docno_uq;
--           DROP INDEX IF EXISTS scm.<table>_migrated_no_stock_idx;
--           ALTER TABLE scm.<table> DROP COLUMN IF EXISTS linked_ac_docno;
--           ALTER TABLE scm.<table> DROP COLUMN IF EXISTS migrated_no_stock;
--           Safe while no migrated invoice exists — the columns default false
--           and empty, so dropping them takes nothing away. It is NOT safe once
--           create-migrated-invoices.mjs has run: migrated_no_stock is the only
--           thing telling a carried-over invoice from a real one, and without it
--           postPiAccounting / postSiRevenue / applyCustomerCreditToSi stop
--           refusing and would book money AutoCount already booked. Reverse
--           BEFORE the converter, or not at all.
--
