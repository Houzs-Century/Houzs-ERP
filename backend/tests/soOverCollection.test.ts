/* OVER-COLLECTION on a sales order (owner 2026-08-16).
   ...
   run 31938591487, prod audit log):
     ...
     10:05:31  CREATE          localTotalCenti = 400000
     10:05:37  ADD_PAYMENT     200000
     08:26:22  UPDATE_LINE     JAGER-(K) unitPriceCenti 0 -> 25000,
...
import { soPaidCenti, soBalanceCenti, soOutstandingCenti } from '../src/scm/shared/so-outstanding';

/* Same shape the SO detail page builds via soPaidInputsOf. RM 4,000 order. */
const order = {
  totalRevenueCenti: 400_000,
...
describe('the signed balance a human is shown', () => {
  test('an exact payment still balances to zero', () => {
    expect(soBalanceCenti({ ...order, ledgerPaidCenti: 400_000 })).toBe(0);
  });

  test('an under-payment is positive — the ordinary case is unchanged', () => {
    expect(soBalanceCenti({ ...order, ledgerPaidCenti: 200_000 })).toBe(200_000);
  });
  ...
  test('an over-collection is negative by EXACTLY the excess, in centi', () => {
    expect(soBalanceCenti({ ...order, ledgerPaidCenti: 425_000 })).toBe(-25_000);
  });

  test('the excess is exact at one sen, not rounded or floored away', () => {
    expect(soBalanceCenti({ ...order, ledgerPaidCenti: 400_001 })).toBe(-1);
  });
  ...
  test('a legacy header deposit over-collects too', () => {
    const legacyDeposit = { ...order, headerDepositCenti: 425_000, depositInLedger: false };
    expect(soPaidCenti(legacyDeposit)).toBe(425_000);
    expect(soBalanceCenti(legacyDeposit)).toBe(-25_000);
  });

  test('a deposit already in the ledger is not counted twice into a false credit', () => {
    expect(soBalanceCenti({
      ...order, headerDepositCenti: 400_000, ledgerPaidCenti: 400_000, depositInLedger: true,
    })).toBe(0);
  });

  /* THE GUARD THAT KEEPS 2,121 PRODUCTION ORDERS OUT OF THE RED. total_revenue_centi
     is 0 on 2,687 of prod's 2,824 live orders — every AutoCount import, where
     the real total sits in local_total_centi (probe run 31938735652 section b).
  ...
  test('a zero total answers 0, NOT a huge negative, even with money against it', () => {
    expect(soBalanceCenti({ ...order, totalRevenueCenti: 0, ledgerPaidCenti: 990_000 })).toBe(0);
    expect(soBalanceCenti({ ...order, totalRevenueCenti: 0, ledgerPaidCenti: 0 })).toBe(0);
  });
});
...
describe('the clamped rule the AutoCount write-back still uses', () => {
  test('an overpaid order writes back 0, never negative', () => {
    expect(soOutstandingCenti({ ...order, ledgerPaidCenti: 425_000 })).toBe(0);
  });

  test('the two rules agree on every non-over-collected order', () => {
    for (const paid of [0, 1, 199_999, 200_000, 399_999, 400_000]) {
      expect(soOutstandingCenti({ ...order, ledgerPaidCenti: paid }))
        .toBe(soBalanceCenti({ ...order, ledgerPaidCenti: paid }));
    }
  });

  test('and diverge only past the total, which is the whole point of two names', () => {
    const over = { ...order, ledgerPaidCenti: 425_000 };
    expect(soOutstandingCenti(over)).toBe(0);
    expect(soBalanceCenti(over)).toBe(-25_000);
  });
});
...
describe('recording a payment cannot touch the order itself', () => {
  ...
  /* The columns the owner watched move. None of them may be written from a
     payment route — `total_revenue_centi` and `local_total_centi` are read by
  ...
  test('the guard removal did not take the amount validation with it', () => {
    ...
    expect(routeSource).toContain('amountCenti:        z.number().int().nonnegative()');
  });
});
