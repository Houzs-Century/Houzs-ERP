...
/* ── One entry, one claim per bank account (docs/bugs/0917) ──────────────────
   ...
   SAME bank is still refused, and a bank the entry never touches cannot
   claim it at all. */
describe('an internal transfer is one entry on two statements', () => {
  const TRANSFER = [
    ...
    /* A voucher that touches Maybank alone. */
  ...
  const world = () => harness({ acc_bank_statement_config: [MBB_ACCOUNT, HLB_ACCOUNT], v_gl_entries: TRANSFER.map((e) => ({ ...e })) });
  const bothUploaded = async (app: Hono) => {
    ...
    const mbb = await (await upload(app, { fileName: 'mbb-july.csv', content: MBB_JULY })).json() as any;
