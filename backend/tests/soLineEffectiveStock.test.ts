 ...
 * touched here — soReadinessRemark.test.ts owns that and it was the owner's own
 * ruling the same day; what is pinned is that the rule is fed the same input
 * the pill shows.
 */
...
describe('the rollup the owner reads, fed the two ways', () => {
  ...
  const remarkFromStored = (l: typeof so002 | typeof so003) =>
    summariseReadiness([{ item_group: l.item_group, item_code: l.item_code, stock_status: l.stored }]).stockRemark;
  const remarkFromEffective = (l: typeof so002 | typeof so003) =>
    summariseReadiness([{
      item_group: l.item_group,
      item_code: l.item_code,
      stock_status: effectiveLineStockStatus(l.stored, l.live),
    }]).stockRemark;

  it('2608-002: the stored rollup is the lie the owner saw, the effective one is not', () => {
    expect(remarkFromStored(so002)).toBe('SHORT: MATTRESS');   // what the board printed
    expect(remarkFromEffective(so002)).toBe('READY');          // what the drill-down showed
  });

  it('2608-003 does NOT move — an incoming PO is not stock, and SHORT: BEDFRAME was correct', () => {
    expect(remarkFromStored(so003)).toBe('SHORT: BEDFRAME');
    expect(remarkFromEffective(so003)).toBe('SHORT: BEDFRAME');
  });
  ...
  it('the two orders together still roll up independently', () => {
    ...
    expect(both.stockRemark).toBe('SHORT: BEDFRAME');
