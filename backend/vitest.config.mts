...
export default defineConfig(async () => {
  ...
  return {
    ...
    test: {
      ...
      // is where most of the suite's 565s went. The split is generated by
      // `npm run gen:test-projects` and gated by `npm run audit:test-projects`
      // so a new test cannot quietly land in the expensive project.
