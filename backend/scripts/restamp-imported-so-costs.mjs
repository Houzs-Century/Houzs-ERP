...
// costs), so every line carries unit_cost_sen = 0 and the list's category
// cost columns (header aggregates) read 0.
//
// For HOUZS lines with unit_cost_sen = 0 whose product now carries a cost:
...
// line_cost = unit x qty. Then, for headers whose total_cost_sen = 0,
...
try {
  ...
  const sos = await sql`SELECT doc_no, total_revenue_sen, total_cost_sen FROM scm.mfg_sales_orders
                        WHERE company_id = ${cid} AND total_cost_sen = 0
                        ORDER BY doc_no`;
  ...
  for (const so of sos) {
    const runOne = async (tx) => {
      const lines = await tx`SELECT id, item_code, qty, variants, unit_cost_sen FROM scm.mfg_sales_order_items WHERE company_id = ${cid} AND doc_no = ${so.doc_no}`;
      const agg = { mattress_sofa: 0, bedframe: 0, accessories: 0, service: 0, others: 0 };
      let anyStamp = false;
      for (const l of lines) {
        let uc = l.unit_cost_sen || 0;
        if (uc === 0) {
          ...
          if (uc > 0) {
            ...
            if (APPLY) await tx`UPDATE scm.mfg_sales_order_items
              SET unit_cost_sen = ${uc}, line_cost_sen = ${uc * (l.qty || 1)}
      ...
      if ((anyStamp || total > 0) && so.total_cost_sen === 0 && total > 0) {
        headers++;
        if (APPLY) await tx`UPDATE scm.mfg_sales_orders SET
            mattress_sofa_cost_sen = ${agg.mattress_sofa}, bedframe_cost_sen = ${agg.bedframe},
            accessories_cost_sen = ${agg.accessories}, service_cost_sen = ${agg.service},
            others_cost_sen = ${agg.others}, total_cost_sen = ${total},
            total_margin_sen = ${(so.total_revenue_sen || 0) - total}, updated_at = ${now}
