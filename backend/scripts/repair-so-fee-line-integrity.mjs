  ...
  for (const cand of candidates) {
    ...
    try {
      await pg.begin(async (sql) => {
        ...
        const isCross = !!src;
        const itemCode = isCross ? "SVC-DELIVERY-CROSS" : "SVC-DELIVERY";
        console.log(`${so.doc_no}  header fee ${rm(fee)}  →  materialise ${itemCode} qty 1 ${rm(fee)}${isCross ? ` (follow-up of ${src})` : ""}`);

        /* The row mirrors recomputeDeliveryFeeCore's build field-for-field
           (mfg-sales-orders.ts) so a repaired line is indistinguishable from a
           derived one. company_id is NOT passed — the RPC reads it from the SO
           header (mig 0214). */
        const keptMax = agg.kept_max_line_no == null ? null : Number(agg.kept_max_line_no);
        const row = {
          doc_no: so.doc_no,
          line_no: keptMax == null ? null : keptMax + 1,
          line_date: todayMyt(),
          debtor_name: so.debtor_name ?? null,
          item_group: "service",
          item_code: itemCode,
          description: isCross ? "Cross-category delivery" : "Delivery fee",
          description2: null,
          remark: isCross && src ? `Follow-up of ${src}` : null,
          uom: "UNIT",
          qty: 1,
          ...
          line_delivery_date: so.customer_delivery_date ?? null,
        ...
        /* THE one write path for SVC-DELIVERY* lines — the same 0214 RPC every
           ...
           the header stamp is a no-op re-write of what it already says. The
           rows ride as a text parameter cast to jsonb (json→jsonb has no
           implicit cast for function resolution). */
        await sql`SELECT scm.rebuild_mfg_so_delivery_lines(
          ${so.doc_no}, ${src}, ${fee}, ${JSON.stringify([row])}::jsonb)`;
        ...
        // recordSoAudit writes; everything else defaults).
        await sql`
