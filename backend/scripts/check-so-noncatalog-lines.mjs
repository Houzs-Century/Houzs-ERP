...
/* The rule is IMPORTED, never re-typed. This script used to carry a FIFTH
   hand-copy of the axes table whose helper took no itemCode at all, so the
   DIVAN ONLY / adjustable / seatless exemptions could not apply and every one
   of those lines was reported as defective — an audit judging by a stricter
   rule than the app enforces. tests/variantAxesMirror.test.ts pins this module
   against the TypeScript source. */
import { missingConfirmVariantAxes } from "./lib/variant-axes.mjs";
...
try {
  ...
  for (const r of rows) {
    ...
    for (const ln of lines) {
      ...
      /* itemCode is what decides the exemptions — pass the real one. The
         previous local helper had no such parameter, which is exactly how a
         DIVAN ONLY line came to be reported as missing a Gap it cannot have. */
      const missing = missingConfirmVariantAxes(ln.grp, ln.variants, code).map((a) => a.label);
