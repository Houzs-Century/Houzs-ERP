...
async function main() {
  ...
  const [hdr] = await sql`SELECT COUNT(*) total,
      COUNT(*) FILTER (WHERE internal_expected_dd IS NOT NULL) processed,
  ...
  const stuck = await sql`
    ...
    WHERE h.company_id = ${CO} AND h.internal_expected_dd IS NOT NULL
  ...
  const bfMismatch = await sql`
    WITH bf_need AS (
      ...
      WHERE h.company_id = ${CO} AND h.internal_expected_dd IS NOT NULL AND i.item_group = 'bedframe'
