...
const TYPES = [
  ...
  },
  {
    ...
    /* READ THE KEY, do not hard-code its absence. This selected `NULL::bigint`
       because migration 0280 added `grn_items.linked_ac_dtlkey` and nothing
       ever filled it — 0 of 636 rows, so the constant and the column agreed and
       the difference was invisible. It stopped being invisible the moment the
       owner asked 「为什么会这样行号不一样呢？」: with the column hard-coded to NULL
       the checker was FORCED to pair by quantity and position on every goods
       receipt, for ever, even after the keys arrived. That guess is what
       produced the 34 item-code "differences" this type reports as the
       checker's own, and it is the same shape that put a REGAL in front of a
       customer whose book line says TRION. backfill-ac-downstream-line-keys.mjs
       fills the column; this reads it. Every other type here already does. */
