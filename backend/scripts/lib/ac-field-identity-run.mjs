...
export function loadAcFieldSide(dataDir, book) {
  ...
  /* DO is listed here but is EMPTY on the committed cut: the `hdr` section of
     export-ac-reimport.py grew a DO lane on 2026-09-07 and this file is only
     regenerated by a re-cut. Listing it now costs nothing (a missing
     `do_fields` leaves the map empty and `enrich` returns immediately) and means
     the re-cut lands the DO currency without another code change. */
  const hdrBy = { SO: new Map(), PO: new Map(), DO: new Map() };
  if (hdrCut?.rows) {
    for (const [kind, fieldsKey, rowsKey] of [["SO", "so_fields", "so"], ["PO", "po_fields", "po"], ["DO", "do_fields", "do"]]) {
