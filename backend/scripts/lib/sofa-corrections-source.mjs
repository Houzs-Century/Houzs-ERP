...
export const CORRECTION_FILES = [
  ...
  /* The SUPPLIER'S OWN LISTING, given by the owner 2026-09-10 and named by him
     the authority on a PROCEEDED order (「supplier的肯定对的 基本上你可以跟」).
     Generated by propose-supplier-sofa-corrections.mjs from production run
     34502972739, which EXCLUDED the one purchase order we amended after their
     export was cut, KEPT the drawing's order on four builds the supplier merely
     lists from the other end, and REPORTED four it genuinely refutes - those
     four earlier entries are superseded in place, on his ruling
     「ok 四张跟supplier的」. The name carries no date: the `only` filter is a
     substring match its own test pins at one round. */
  "sofa-compartment-corrections-supplier-listing.json",
