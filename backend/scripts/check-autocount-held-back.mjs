...
//                  two are what decide whether a rebuild is refused
//                  (docs/bugs/0609).
//
...
try {
  ...
  for (const { s, t } of docNoTables) {
    ...
    controlHits += row.b;
    if (row.a > 0 || row.b > 0) {
      hitLines.push(`   - ${s}.${t}: '${SEARCH}' ${row.a}   '${CONTROL}' ${row.b}   (rows ${row.n})`);
    }
  }
  ...
  const wanted = ["id", "doc_no", "company_id", "status", "cancelled", "doc_date",
  ...
  if (!so) {
    ...
    out(`   doc_date        ${day(so.doc_date)}   created ${day(so.created_at)}   updated ${day(so.updated_at)}`);
    ...
    if (keyless.length) {
      /* Positions, not item codes: a position is enough to open the document
         and is not content. */
      out(`   keyless line positions (1-based, live lines in document order): ${keyless.join(", ")}`);
    }

    /* THE REBUILD GATE (docs/bugs/0609). Mirrors poRaisedFromSo exactly —
    ...
    if (ids.length) {
      try {
        /* Joined through a SUBQUERY on the SO's own lines rather than by
           shipping the id list back as a parameter: the key types across these
           tables are not uniform, and `uuid = ANY(text[])` fails as an absent
           OPERATOR — which would surface as "no purchase order", the one wrong
        ...
        const [c] = await pg`
          SELECT count(*)::int AS n FROM scm.purchase_order_items
           WHERE so_item_id IN (
             SELECT id FROM scm.mfg_sales_order_items WHERE doc_no = ${DOC})`;
        poRaised = c.n > 0;
        out(`   purchase lines raised from this SO: ${c.n}${poDocs.length ? ` (PO ${poDocs.join(", ")})` : ""}`);
      } catch (e) {
    ...
    /* The ordinary downstream lock, for contrast: it counts DO and SI only —
       never a purchase order, which is the whole point of 0609. */
    const downstream = {};
