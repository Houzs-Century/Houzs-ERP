...
async function main() {
  ...
  for (const c of DATA.builds) {
    ...
    for (const doc of docs) {
      ...
      if (c.desc2Match || c.dtlKey) {
        const pick = selectBuildRows(rows, c.desc2Match, undefined, { dtlKey: c.dtlKey });
        /* A build may name its BOOK LINE instead of its text — see the header
           of lib/sofa-desc2-match.mjs. A key no row on THIS document carries is
           not necessarily wrong: one entry can name a sales order and its
           purchase order, and those are two different AutoCount lines with two
           different keys, so the entry lists both and only one belongs here.
           What is never allowed is falling back to the text, which is why the
           row set is empty either way. */
        if (pick.verdict === "key-missing") {
          log(`  ${doc}: ${pick.how} — skipped, this document is not the one that key names`);
          continue;
        }
        if (pick.verdict === "line-key") log(`  ${doc}: ${pick.how} — selected by AutoCount line key, not by text`);
      ...
      verify.push({ doc, isPo, poId, needle: c.desc2Match, dtlKey: c.dtlKey, want, copies: sofas.length, money: before, source: c.source });
...
async function verifyOnFreshConnection(items) {
  ...
  for (const it of items) {
    const rows = it.isPo
      ? await v`SELECT i.id, i.item_code AS code, i.qty, i.unit_price_sen, i.line_total_sen AS total,
                       i.description2, i.linked_ac_dtlkey, i.so_item_id
                  FROM scm.purchase_order_items i
                 WHERE i.purchase_order_id = ${it.poId} AND i.item_group = 'sofa' ORDER BY i.id`
      : await v`SELECT i.id, i.item_code AS code, i.qty, i.unit_price_sen, i.total_sen AS total,
                       i.description2, i.linked_ac_dtlkey
                  FROM scm.mfg_sales_order_items i
                  JOIN scm.mfg_sales_orders h ON h.doc_no = i.doc_no
                 WHERE h.company_id = ${CO} AND i.doc_no = ${it.doc} AND i.item_group = 'sofa' ORDER BY i.line_no`;
    /* Re-read the build the SAME way it was selected. Selecting it by text when
       it was written by key would verify a different set of rows — the check
       that answers a different question, which this file has paid for once. */
    const mine =
      it.needle || it.dtlKey
        ? selectBuildRows(rows, it.needle, undefined, { dtlKey: it.dtlKey }).rows
        : rows;
    const want = [];
