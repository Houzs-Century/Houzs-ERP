...
/* Census the AutoCount party codes the ERP holds. READ-ONLY: SELECT only, no
   ...
   and once, across companies:
     4. any code held by more than one ERP record, in either table
...
/* "Reached AutoCount" has two independent witnesses and a document only has to
   satisfy one. The cross-reference column is written back onto the ERP row by
   the drain; a sent outbox row is the queue's own record. Requiring BOTH would
   under-count the purchase orders imported before the outbox table existed
   (0277 notes purchase_orders.linked_ac_docno predates it, added at runtime by
   import-ac-outstanding-po.mjs); requiring only the column would miss anything
   whose push landed but whose repoint did not. */
...
async function supplierSection(co, supCols, poCols, supCode, hasOutbox) {
  ...
  const rows = canJoin
    ...
    : await sql`
        SELECT btrim(s.code) AS code, s.name AS name, NULL AS po_all, NULL AS po_ac
        FROM scm.suppliers s
        WHERE nullif(btrim(s.code), '') IS NOT NULL ${byCo}
        ORDER BY btrim(s.code), s.name`;
  ...
  out(`   ${rows.length} supplier(s) with a code`
    + (canJoin ? `; ${used} used on a purchase order that reached AutoCount.` : '; usage not measurable.'));
  out();
  out(`   ${pad('CODE', 18)}${pad('ERP NAME', 54)}${pad('POs', 7)}IN AUTOCOUNT`);
  out(rule(18, 54, 7, 13));
  for (const r of capped(rows, 'suppliers with a code')) {
    const mark = canJoin ? (Number(r.po_ac) > 0 ? `YES (${r.po_ac})` : 'no') : '?';
    out(`   ${pad(r.code, 18)}${pad(clip(r.name, 52), 54)}${pad(r.po_all ?? '?', 7)}${mark}`);
  }
...
async function customerSection(co, cusCols, soCols, cusCode, hasOutbox) {
  ...
  const rows = canJoin
    ...
    : await sql`
        SELECT btrim(c.${sql(cusCode)}) AS code, c.name AS name, NULL AS so_all, NULL AS so_ac
        FROM scm.customers c
        WHERE nullif(btrim(c.${sql(cusCode)}), '') IS NOT NULL ${byCo}
        ORDER BY btrim(c.${sql(cusCode)}), c.name`;
  ...
  out(`   ${rows.length} customer(s) with a code`
    + (canJoin ? `; ${used} used on a sales order that reached AutoCount.` : '; usage not measurable.'));
  out();
  out(`   ${pad('CODE', 18)}${pad('ERP NAME', 54)}${pad('SOs', 7)}IN AUTOCOUNT`);
  out(rule(18, 54, 7, 13));
  for (const r of capped(rows, 'customers with a code')) {
    const mark = canJoin ? (Number(r.so_ac) > 0 ? `YES (${r.so_ac})` : 'no') : '?';
    out(`   ${pad(r.code, 18)}${pad(clip(r.name, 52), 54)}${pad(r.so_all ?? '?', 7)}${mark}`);
  }
  if (!rows.length) out('   (none)');
}
...
async function documentSection(co, supCols, poCols, soCols, supCode, hasOutbox) {
  ...
  if (poCols.size && supCols.size && supCode && poCols.has('supplier_id') && poCols.has('linked_ac_docno')) {
    ...
    const rows = await sql`
      ...
      ORDER BY btrim(coalesce(s.code, '')), po.po_number`;
    out();
    out(`   PURCHASE ORDERS IN AUTOCOUNT: ${rows.length}`);
    out(`   ${pad('CREDITOR', 18)}${pad('ERP SUPPLIER NAME', 44)}${pad('ERP PO', 22)}AC DOC`);
    out(rule(18, 44, 22, 12));
    for (const r of capped(rows, 'AutoCount-linked purchase orders')) {
      out(`   ${pad(r.code || '(none)', 18)}${pad(clip(r.name, 42), 44)}${pad(clip(r.doc, 20), 22)}${r.ac ?? '(outbox only)'}`);
    }
    if (!rows.length) out('   (none)');
  } else {
  ...
  if (soCols.size && soCols.has('debtor_code') && soCols.has('linked_ac_docno')) {
    ...
    const rows = await sql`
      SELECT btrim(coalesce(so.debtor_code, '')) AS code, count(*) AS n
    ...
    out();
    out(`   SALES ORDERS IN AUTOCOUNT: ${total}. debtor_code values on the ERP rows:`);
    for (const r of rows) out(`     ${pad(r.code || '(empty)', 18)}${r.n}`);
    if (!rows.length) out('     (none)');
