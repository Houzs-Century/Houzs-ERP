...
try {
  ...
  // 14) Is the residual 0 the SALES SCOPE for the signed-in user (Lim, id=4)?
  //     Resolve view-all + self/downline staff uuids -> SO count, guarded.
  try {
    const [{ n: staffReadable }] = await (async () => {
      try { await pg.unsafe("SET ROLE service_role"); const r = await pg`SELECT count(*)::int AS n FROM scm.staff`; return r; }
      finally { await pg.unsafe("RESET ROLE"); }
    })();
    notice(`AS service_role: scm.staff readable, rows=${staffReadable}`);
  } catch (e) { notice(`AS service_role scm.staff: ERROR ${e.code ?? ""} ${e.message}`); }

  try {
    const lims = await pg`SELECT id, name, role_id, position_id, manager_id, status FROM public.users WHERE name ILIKE '%lim%' OR email ILIKE '%lim%' ORDER BY id LIMIT 12`;
    notice(`---- users matching 'lim': ${lims.length} ----`);
    for (const u of lims) {
      // position + role names (view-all = position in Super Admin/Sales Director/Finance Manager, or a '*'/scm.so.view_all permission)
      let posName = null, roleName = null, rolePerms = null;
      try { const p = await pg`SELECT to_jsonb(x) j FROM scm.positions x WHERE x.id = ${u.position_id}`; posName = p[0]?.j?.name ?? p[0]?.j?.title ?? null; } catch {}
      if (posName === null) { try { const p = await pg`SELECT to_jsonb(x) j FROM public.positions x WHERE x.id = ${u.position_id}`; posName = p[0]?.j?.name ?? p[0]?.j?.title ?? null; } catch {} }
      try { const r = await pg`SELECT to_jsonb(x) j FROM public.roles x WHERE x.id = ${u.role_id}`; const j = r[0]?.j ?? {}; roleName = j.name ?? j.code ?? null; rolePerms = j.permissions ?? j.perms ?? null; } catch (e) { roleName = `(roles err ${e.code})`; }
      notice(`user id=${u.id} name=${u.name} status=${u.status} manager_id=${u.manager_id} position=${JSON.stringify(posName)} role=${JSON.stringify(roleName)} rolePerms=${rolePerms ? JSON.stringify(rolePerms).slice(0,120) : "n/a"}`);
      // self + downline -> staff uuids -> SO count (company 1)
      try {
        const sub = await pg`WITH RECURSIVE t(id) AS (SELECT ${u.id}::int UNION SELECT us.id FROM public.users us JOIN t ON us.manager_id = t.id) SELECT array_agg(DISTINCT id) ids FROM t`;
        const ids = sub[0].ids ?? [];
        const st = await pg`SELECT COALESCE(array_agg(id),'{}') sids FROM scm.staff WHERE user_id = ANY(${ids})`;
        const sids = st[0].sids ?? [];
        let inScope = 0;
        if (sids.length) { const c = await pg`SELECT count(*)::int n FROM scm.mfg_sales_orders_with_payment_totals WHERE company_id=1 AND salesperson_id = ANY(${sids}::uuid[])`; inScope = c[0].n; }
        notice(`  scope: subtree_users=${ids.length} staff_uuids=${sids.length} -> SO rows(company1) in scope = ${sids.length? inScope : "0 (no staff uuid => resolveSalesScopeIds returns MATCH_NOTHING)"}`);
      } catch (e) { notice(`  scope ERROR ${e.code ?? ""} ${e.message}`); }
    }
