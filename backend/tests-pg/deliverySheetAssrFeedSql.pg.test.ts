...
async function resetFixture(s: Sql): Promise<void> {
  ...
  await s.unsafe(`
    ...
    VALUES
      ...
      -- Only own-team pickup dated → 1 leg. Native-order doc_no (HC-SO-...).
...
describePg('HC Delivery sheet ASSR leg feed SQL — real Postgres', () => {
  ...
  test('the mapper expands the page into its legs', async () => {
    ...
    // date write-back to the case, not the DO number the SO feed puts there.
    expect(del).toMatchObject({ TransferTo: 'ASSR/2609-010', SalesExemptionExpiryDate: '2026-09-25', Region: 'WEST' });
  });
