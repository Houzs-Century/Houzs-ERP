 ...
 * copies: backend/src/scm/shared/po-line-import.ts is what the server classifies
...
describe('the two copies of po-line-import are the same file', () => {
  ...
  test('backend/src/scm/shared/po-line-import.ts is byte-identical to this one', () => {
    expect(norm('../backend/src/scm/shared/po-line-import.ts')).toBe(norm('src/vendor/shared/po-line-import.ts'));
  });

  test('this pin is not vacuous — both files are real and carry the mapping', () => {
    for (const p of ['src/vendor/shared/po-line-import.ts', '../backend/src/scm/shared/po-line-import.ts']) {
      const t = norm(p);
      expect(t.length).toBeGreaterThan(1000);
      expect(t).toContain('PO_LINE_IMPORT_FIELDS');
      expect(t).toContain('supplier_delivery_date_2');
    }
