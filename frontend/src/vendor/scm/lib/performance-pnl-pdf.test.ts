...
describe('performanceTables', () => {
  it('lays a row per group with GP and %, a Total, then the summary from gross profit to net', () => {
    ...
    expect(t.summary[0]!).toEqual({ kind: 'total', label: 'Gross profit', amountSen: 161000, pct: 43.2, depth: 0 });
    expect(t.summary[1]!).toEqual({ kind: 'category', label: 'OTHER INCOMES', amountSen: 50000, pct: 13.4, depth: 1 });
    expect(t.summary[2]!).toEqual({ kind: 'row', label: '590-0000 — RENT RECEIVED', amountSen: 50000, pct: 13.4, depth: 2 });
    expect(t.summary[3]!).toEqual({ kind: 'total', label: 'Total other income (as booked)', amountSen: 50000, pct: 13.4, depth: 0 });
    ...
    expect(t.summary[5]!).toEqual({ kind: 'row', label: '900-R048 — RENTAL OF SHOWROOM', amountSen: 4500000, pct: 1206.4, depth: 1 });
    expect(t.summary[6]!).toEqual({ kind: 'total', label: 'Total expenses (operating expense at 16.00% + as booked)', amountSen: 4556000, pct: 1221.4, depth: 0 });
    expect(t.summary[7]!).toEqual({ kind: 'net', label: 'NET PERFORMANCE', amountSen: -4345000, pct: -1164.9, depth: 0 });
  });
