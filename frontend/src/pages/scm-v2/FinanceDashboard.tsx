...
// ACTUALS (petrol bars) beside the Forecast P&L's TARGETS (red dashed line),
...
/** The chart's dress (owner 2026-09-22: 跟 app 的 petrol 主题, then 我不要黑色):
    ...
    petrol, the app's primary — money in and the assets wear it too; the
    orders' PERFORMANCE is the secondary blue, and the product groups stack in
    blues, amber, sage and greys so none of them wears the actual's petrol;
    the forecast stays red so the target jumps out; one colour a group across
    every card, Bedding = mattress + bedframe. */
const PETROL = '#16695f';   // --fg-accent (--c-orange, Theme C primary)
const FOREST = '#2F5D4F';   // --c-secondary-a
...
const GROUP_COLORS: Record<string, string> = {
  sofa: BLUE, bedding: SKY, accessories: SAGE, dining: AMBER, others: MIST, service: SOFT,
  mattress: SKY, bedframe: SKY, accessory: SAGE,
};
...
/** The Performance P&L's blue beside the ledger's petrol. */
const PERF = BLUE;
...
/** Owner 2026-09-22: actual (the ledger, petrol) and performance (the orders,
...
const PerformanceCard = ({ periods, groups }: { periods: DashboardPeriod[]; groups: Array<{ key: string; label: string }> }) => {
  ...
  const bars: ChartSeries[] = metric === 'sales'
    ? [{ key: 'actual', label: 'Actual sales (P&L)', color: ACTUAL, values: actualSales, tips: periods.map((p, i) => (actualSales[i] == null ? null : `${labelOf(p)} · Actual sales (P&L) · ${groupLabel}: RM ${fmtSenPlain(actualSales[i])}`)) }, ...perfBars]
    : perfBars;
...
const CashFlowCard = ({ periods }: { periods: DashboardPeriod[] }) => {
  ...
  const color = tab === 'in' ? IN : tab === 'out' ? OUT : NET;
  ...
  return (
    <div className="space-y-4">
      <PageHeader eyebrow="Finance · Forecasting" title="Dashboard"
        description="The statements' actuals (petrol) beside the Forecast P&L's targets (red, dashed), per month or quarter, from the first month with sales. Every figure is the report's own — the P&L, the Cash Flow, the Performance P&L, the balance sheet — read once per period. Nothing here writes." />
