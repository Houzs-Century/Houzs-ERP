...
export function deliveryOrderRowMenu<R extends StatusRow>(h: {
  ...
  return (r) => buildRowMenu(
    ...
    /* THE ONE EXCEPTION to "no status entries here", and it is not one: a hold
       is not a status. The rule above refuses status moves because the DO is the
       document where a status move writes an inventory OUT. A hold writes no
       movement of any kind — mig 0324 gave the DO four columns and left
       scm.do_status untouched — so the objection does not apply to it. */
    holdEntries(r, h.setHold),
  );
}
